package id.diazwijaya.blue_tsc_printer;
...
public class BlueTscPrinterPlugin implements FlutterPlugin, ActivityAware,MethodCallHandler, RequestPermissionsResultListener {

  private static final String TAG = "BtTscPrinterPlugin";
  private static final String NAMESPACE = "blue_tsc_printer";
    ...
    switch (call.method) {
	  ...
	  case "printBarcode":
        if (arguments.containsKey("textToBar")) {
          ...
          int width = (int) arguments.get("width");
          int height = (int) arguments.get("height");
          printBarcode(result, textToBar, width, height);
        } else {
          result.error("invalid_argument", "argument 'textToBar' not found", null);
        }
        break;	
  ...
  private void printQRcode(Result result, String textToQR, int width, int height, int align) {
    ...
    try {
      ...
      if (bmp != null) {
        ...
        Log.e("Print QRCode error", "the file isn't exists");
      }
      result.success(true);
    } catch (Exception ex) {
      Log.e(TAG, ex.getMessage(), ex);
  ...
  private void printBarcode(Result result, String textToBar, int width, int height) {
    MultiFormatWriter multiFormatWriter = new MultiFormatWriter();
    if (THREAD == null) {
      result.error("write_error", "not connected", null);
      return;
    ...
    try {
	  THREAD.write(PrinterCommands.ESC_ALIGN_CENTER);
	  BitMatrix bitMatrix = multiFormatWriter.encode(textToBar, BarcodeFormat.CODE_128, width, height);
      BarcodeEncoder barcodeEncoder = new BarcodeEncoder();
      Bitmap bmp = barcodeEncoder.createBitmap(bitMatrix);
      if (bmp != null) {
        byte[] command = Utils.decodeBitmap(bmp);
        ...
        Log.e("Print Barcode error", "the file isn't exists");
      }
      result.success(true);
    } catch (Exception ex) {
      Log.e(TAG, ex.getMessage(), ex);
